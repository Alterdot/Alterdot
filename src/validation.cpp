...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams, bool checkHeader)
{
    ...
    // Check the header
    if (checkHeader && !CheckProofOfWork(block.GetPoWAlgoHash(Params().GetConsensus()), block.nBits, consensusParams))
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW)
{
    ...
    if (fCheckPOW && !CheckProofOfWork(block.GetPoWAlgoHash(Params().GetConsensus()), block.nBits, consensusParams))
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    if(Params().NetworkIDString() == CBaseChainParams::MAIN && nHeight <= 68589){
        ...
        unsigned int nBitsNext = GetNextWorkRequired(pindexPrev, &block, consensusParams, 0);
        ...
        if (block.nBits != GetNextWorkRequired(pindexPrev, &block, consensusParams, 0))
