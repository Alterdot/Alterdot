...
void PrepareShutdown()
{
    ...
    GenerateAlterdot(false, 0, Params());
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    std::string debugCategories = "addrman, alert, bench, cmpctblock, coindb, db, http, leveldb, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                                  "alterdot (or specifically: chainlocks, gobject, instantsend, keepass, llmq, llmq-dkg, llmq-sigs, masternode, mnpayments, mnsync, privatesend, spork)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode", strprintf(_("Disable all Alterdot specific functionality (Masternodes, PrivateSend, InstantSend, Governance) (0-1, default: %u)"), 0));
    strUsage += HelpMessageOpt("-sporkaddr=<alterdotaddress>", strprintf(_("Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.")));
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N ADOT anonymized (%u-%u, default: %u)"), MIN_PRIVATESEND_AMOUNT, MAX_PRIVATESEND_AMOUNT, DEFAULT_PRIVATESEND_AMOUNT));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/alterdot/alterdot>";
    const std::string URL_WEBSITE = "<https://alterdot.org>";
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    // the BlockchainDNS has to be reindexed after importing blocks from a file or if the reindexing flag is set
    ...
    if (fReindexBdns || !pbdnsdb->CheckVersion())
        if (!pbdnsdb->WriteReindexing(true)) {
            pbdnsdb->WriteCorruptionState(true); // shouldn't happen but in case writing the reindex flag fails the corruption flag should get set
...
/** Sanity checks
 *  Ensure that Alterdot is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Alterdot process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    int64_t nBDNSDBCache = nTotalCache / 32;
    nTotalCache -= nBDNSDBCache;
    ...
    LogPrintf("* Using %.1fMiB for blockchain domain name system database\n", nBDNSDBCache * (1.0 / 1024 / 1024));
    ...
    while (!fLoaded && !fRequestShutdown) {
        ...
        do {
            try {
                ...
                pbdnsdb = new CBDNSDB(nBDNSDBCache, false, false);
    ...
    // ********************************************************* Step 7b: check lite mode and load sporks
    ...
    // ********************************************************* Step 10d: schedule Alterdot-specific tasks
