...
//! Discourage users to set fees higher than this amount (in dots) per kB
static const CAmount HIGH_TX_FEE_PER_KB = 0.01 * COIN;
//! -maxtxfee will warn if called with a higher fee than this amount (in dots)
...
static const unsigned int MAX_BLOCKFILE_SIZE = 0x20000000; // 512 MiB TODO_ADOT_FUTURE might lower this
 ...
 *  We have 4 times smaller block times in Alterdot, so we need to push 4 times more invs per 1MB. */
static const unsigned int INVENTORY_BROADCAST_MAX_PER_1MB_BLOCK = 4 * 7 * INVENTORY_BROADCAST_INTERVAL;
/** Block download timeout base, expressed in millionths of the block interval (i.e. 6 min) */
static const int64_t BLOCK_DOWNLOAD_TIMEOUT_BASE = 1000000;
/** Additional block download timeout per parallel downloading peer (i.e. 3 min) */
...
/** Absolute maximum transaction fee (in dots) used by wallet and mempool (rejects high fee in sendrawtransaction) */
...
bool ProcessNewBlockHeaders(const std::vector<std::pair<CBlockHeader, uint256>>& vecHeaderHash, CValidationState& state, const CChainParams& chainparams, const CBlockIndex** ppindex);
...
bool ActivateBestChain(CValidationState& state, const CChainParams& chainparams, std::shared_ptr<const CBlock> pblock = std::shared_ptr<const CBlock>(), const uint256& hashBlock = uint256());
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams, bool checkHeader = true);
...
/** Context-independent validity checks */
bool CheckBlockHeader(const CBlockHeader& block, const uint256& blockHash, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true);
bool CheckBlock(const CBlock& block, const uint256& blockHash, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true, bool fCheckMerkleRoot = true);
