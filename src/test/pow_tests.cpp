...
BOOST_AUTO_TEST_CASE(get_next_work)
{
    ...
    BOOST_CHECK_EQUAL(GetNextWorkRequired(&blockIndexLast, &blockHeader, params, 0), 0x1b1441de); // Block #123457 has 0x1b1441de
    ...
    blockHeader.nTime = 1408732505; // Block #123457
    BOOST_CHECK_EQUAL(GetNextWorkRequired(&blockIndexLast, &blockHeader, paramsdev, 0), 0x1b1441de); // Block #123457 has 0x1b1441de

    // 10x higher target
    blockHeader.nTime = 1408733090; // Block #123457 (10m+1sec)
    BOOST_CHECK_EQUAL(GetNextWorkRequired(&blockIndexLast, &blockHeader, paramsdev, 0), 0x1c00c8f8); // Block #123457 has 0x1c00c8f8
    blockHeader.nTime = 1408733689; // Block #123457 (20m)
    BOOST_CHECK_EQUAL(GetNextWorkRequired(&blockIndexLast, &blockHeader, paramsdev, 0), 0x1c00c8f8); // Block #123457 has 0x1c00c8f8
    // lowest diff possible
    blockHeader.nTime = 1408739690; // Block #123457 (2h+1sec)
    BOOST_CHECK_EQUAL(GetNextWorkRequired(&blockIndexLast, &blockHeader, paramsdev, 0), 0x207fffff); // Block #123457 has 0x207fffff
...
BOOST_AUTO_TEST_CASE(GetBlockProofEquivalentTime_test)
{
    ...
    for (int i = 0; i < 10000; i++) {
        ...
        blocks[i].nHeight = i;
        blocks[i].nTime = 1269211443 + i * params.GetCurrentPowTargetSpacing(i);
