...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total alterdot balance of the wallet\n"
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized alterdot balance of the wallet\n"
...
UniValue debug(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nExamples:\n"
            + HelpExampleCli("debug", "alterdot")
            + HelpExampleRpc("debug", "alterdot+net")
        );
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given alterdot address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The alterdot address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The alterdot address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Alterdot address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are alterdot addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) alterdot address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The alterdot address to use for the signature.\n"
...
UniValue getaddressmempool(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "    \"satoshis\"  (number) The difference of dots\n"
...
UniValue getaddressutxos(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "    \"satoshis\"  (number) The number of dots of the output\n"
...
UniValue getaddressbalance(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"balance\"  (string) The current balance in dots\n"
            "  \"received\"  (string) The total number of dots received (including change)\n"
            "}\n"
            "\nExamples:\n"
...
UniValue registerdomain(const JSONRPCRequest& request)
{
    ...
    if (!request.params[2].isNull()) {
        if (request.params[2].get_str().length() != 34)
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, argument 3 does not have the required length of a Alterdot address (34 characters).");

        payingAddress.SetString(request.params[2].get_str());

        if (!payingAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Alterdot address."));

        // TODO_ADOT_LOW implement payments with script keys

        if (payingAddress.IsScript())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Only pub-key Alterdot addresses are allowed in domain name registrations (no multi-sig)."));
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs) {
        ...
        // price of registering is 0.2 Alterdot or 20 cents so the output has to hold at least that amount
...
UniValue updatedomain(const JSONRPCRequest& request)
{
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs) {
        ...
        // price of updating is 0.01 Alterdot or 1 cent so the output has to hold at least that amount
        // half of it gets burned and the other goes to the miners as transaction fee
        if (nValue >= 1 * CENT) {
