...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    result.push_back(Pair("pow_hash", GetPoWAlgoHash(block).GetHex()));
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of alterdot addresses\n"
            "        \"address\"     (string) alterdot address\n"
            "        ,...\n"
            "     ]\n"
            "  },\n"
...
UniValue getblockchaininfo(const JSONRPCRequest& request)
{
    ...
    //BIP9SoftForkDescPushBack(bip9_softforks, "dip0003", consensusParams, Consensus::DEPLOYMENT_DIP0003); // TODO_ADOT_LOW mention these somehow as we've implemented without signaling
